...
contract Coupon is ERC721, Ownable {
    ...
    */
    ...
    event CouponCreated(
        ...
        string indexed _ifpsURL,
        uint indexed _status
    );
    event Redeemed(
        address indexed _ad,
        uint indexed _couponId,
        uint indexed _status,
        uint tokenId
    );
    event EndDateUpdate(
        uint indexed _couponId,
        uint indexed _newEndDate,
        uint indexed _status
    );
    event EndDateUpdateAndMetadataUpdate(
        uint indexed _couponId,
        bool indexed _combinedEdit,
        uint indexed _status
    );
    event UpdatedMetaInformation(
        string indexed _couponStr,
        uint indexed _couponId,
        uint indexed _status
    );
    event DeactivateCouponState(
        string indexed _CouponNo,
        bool indexed _boolStatus,
        uint indexed _status
    );
    event DeactivateCouponsBulkState(
        string[] indexed _CouponNo,
        bool indexed _boolStatus,
        uint indexed _status
    );
    event ReactivateCouponState(
        string indexed _CouponNo,
        bool indexed _boolStatus,
        uint indexed _status
    );
    event ReactivateCouponsBulkState(
        string[] indexed _CouponNo,
        bool indexed _boolStatus,
        uint indexed _status
    );
    ...
    uint private constant Status_Success = 200;
    uint256 private autoGenerateId = 10000;
    uint256 private autoGenerateNftId = 100;

    // type - String
    string private constant status_1 = "Coupon is Live";
    string private constant status_2 = "Coupon Expired";
    string private constant status_3 = "Coupon not created";
    string private constant status_4 = "Coupon is deactivated";
    ...
    mapping(string => mapping(uint => bool))
        private linkCouponStringAndCouponId;
    ...
    function redeemCoupon(
        ...
        require(
            regDriver.isDriverRegistered(_driverAddress),
            "The address is not registered in the registration contract"
        );
        incrementToken += 1;
        ...
        if (
            linkConfirmation[_driverAddress][
                showCouponStringAndCouponId[_couponId]
            ]
        ) {
            revert AlreadyRedeemed();
        }
        if (couponHealth[_couponId][showCouponStringAndCouponId[_couponId]]) {
            holdUri[idCreate] = couponInformation[
                showCouponStringAndCouponId[_couponId]
            ]._tokenURIOfIPFS;
            ...
            allCouponIdToUser[_driverAddress].push(
                showCouponStringAndCouponId[_couponId]
            );
            linkConfirmation[_driverAddress][
                showCouponStringAndCouponId[_couponId]
            ] = true;
            verifyDriverAddressWithID[_driverAddress][
                showCouponStringAndCouponId[_couponId]
            ] = true;
            couponLinkedIds[showCouponStringAndCouponId[_couponId]].push(
                idCreate
            );
            emit Redeemed(
                _driverAddress,
    ...
    function editEndDate(
        ...
        require(
            _updateEndDate >
                couponInformation[showCouponStringAndCouponId[_couponId]]
                    ._endDate,
            "enter date that is greater than current end date"
        );
        if (couponHealth[_couponId][showCouponStringAndCouponId[_couponId]]) {
            couponInformation[showCouponStringAndCouponId[_couponId]]
                ._endDate = _updateEndDate;
            emit EndDateUpdate(
                showCouponStringAndCouponId[_couponId],
                _updateEndDate,
                Status_Success
     ...
     */
    function editCouponMetadata(
        string memory _couponId,
        string memory _updatedIPFS
    ) external onlyAdmin {
        if (!idCheck[showCouponStringAndCouponId[_couponId]]) {
            revert IdIsNotYetCreated();
        }
     ...
     * @param _updateEndDate - Enter the updated End Date.
     */
    function editEndDateAndMetadataIPFS(
        string memory _couponId,
        uint _updateEndDate,
        string memory _updatedIPFS
    ) external onlyAdmin {
        ...
        if (couponHealth[_couponId][showCouponStringAndCouponId[_couponId]]) {
            ...
            couponInformation[showCouponStringAndCouponId[_couponId]]
                ...
                .push(_updatedIPFS);
            holdUri[idCreate] = _updatedIPFS;
            emit EndDateUpdateAndMetadataUpdate(
                showCouponStringAndCouponId[_couponId],
                true,
                Status_Success
            );
     ...
     */
    function DeactivateCouponsBulk(
        string[] memory _couponId
    ) external onlyAdmin {
        for (uint i = 0; i < _couponId.length; i++) {
            if (
                couponHealth[_couponId[i]][
                    showCouponStringAndCouponId[_couponId[i]]
                ]
            ) {
                couponHealth[_couponId[i]][
                    showCouponStringAndCouponId[_couponId[i]]
     ...
     * @param _couponId - Enter the coupon id // eg get50% or get100%
     */
    function ReactivateCouponsBulk(
        string[] memory _couponId
    ) external onlyAdmin {
        for (uint i = 0; i < _couponId.length; i++) {
            if (
     ...
     */
    function removeCouponLinkToUser(
        uint _id,
        address[] memory _allAddress
    ) external onlyAdmin {
     ...
     */
    function viewAllCreatedTokens()
        external
        view
        returns (TokenSpecification[] memory tokenspec)
    {
        TokenSpecification[] memory tokens = new TokenSpecification[](
            tokenIds.length
        );
     ...
     * @param _couponId - Enter the coupon id.
     */
    function viewCouponValidityToUser(
        address _ad,
        string memory _couponId
    )
        external
        ...
        )
    {
        if (couponHealth[_couponId][showCouponStringAndCouponId[_couponId]]) {
            if (
                linkConfirmation[_ad][showCouponStringAndCouponId[_couponId]] &&
                viewCouponStatus(_couponId)
            ) {
                return (
                    couponInformation[showCouponStringAndCouponId[_couponId]]
                        ._tokenId,
            ...
            if (
                linkConfirmation[_ad][showCouponStringAndCouponId[_couponId]] &&
                viewCouponStatus(_couponId) == false
            ) {
                return (
                    couponInformation[showCouponStringAndCouponId[_couponId]]
                        ._tokenId,
     ...
     */
    function viewLiveCouponsOnly()
        external
        view
        returns (OnlyLive[] memory data)
    {
        ...
        for (uint j = 0; j < allCouponStrings.length; j++) {
            if (
                block.timestamp >
                couponInformation[
                    showCouponStringAndCouponId[allCouponStrings[j]]
                ]._startDate &&
                block.timestamp <
                couponInformation[
                    showCouponStringAndCouponId[allCouponStrings[j]]
                ]._endDate &&
                couponHealth[allCouponStrings[j]][
                    showCouponStringAndCouponId[allCouponStrings[j]]
                ]
            ) {
                livecouponCount++;
        ...
        for (uint k = 0; k < allCouponStrings.length; k++) {
            ...
            if (
                block.timestamp > couponInformation[couponId]._startDate &&
                block.timestamp < couponInformation[couponId]._endDate &&
                couponHealth[allCouponStrings[k]][
                    showCouponStringAndCouponId[allCouponStrings[k]]
                ]
            ) {
     ...
     */
    function allCouponIds()
        external
        view
        returns (uint[] memory allCreatedIds)
    {
        return tokenIds;
    }
     ...
     */
    function allAdminAddresses()
        external
        view
        returns (address[] memory allAdminAddress)
    {
     ...
     */
    function couponsPerAddress(
        address _ad
    )
        external
        view
        returns (uint[] memory allCouponIdsOfUsers, uint totalCoupons)
    {
        return (allCouponIdToUser[_ad], totalCouponCountPerAddress[_ad]);
    }
     ...
     * @param _couponId - Pass the coupon id
     */
    function returnsAllNFTsIds(
        string memory _couponId
    ) external view returns (uint[] memory couponRelatedIds) {
     ...
     * @param _couponId - Pass the coupon id
     */
    function couponCreatedOrNotCreatedStatus(
        string memory _couponId
     ...
     */
    function couponLiveOrNotLive(
        string memory _couponId
    ) external view returns (bool couponStatus) {
