...
contract Ticketing is Initializable, UUPSUpgradeable, OwnableUpgradeable, ERC721Upgradeable{

    // Errors
    error maxLimitExceeded();
    error maxTicketPurchasedPerUser();

    event NewTicketCreated(uint indexed eventID, uint indexed publishTime, uint indexed ticketsCount);
    ...
    function initialize(address _registrationContract, address _evntTokenAddress) external initializer{
      ///@dev as there is no constructor, we need to initialise the OwnableUpgradeable explicitly
        _name = "Ticmet-Ticketting";
        _symbol = "TIC";
