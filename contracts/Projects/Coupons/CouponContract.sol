...
contract Coupon is ERC721, Ownable{
    ...
    */
    ...
    mapping(address => uint[]) private allNFTIdToUser;
    ...
    function redeemCoupon(address _driverAddress, string memory _couponId) external onlyAdmin{
        ...
        if(couponHealth[_couponId][showCouponStringAndCouponId[_couponId]]){
            ...
            allNFTIdToUser[_driverAddress].push(idCreate);
    ...
    /**
     * @notice allNftIdsPerAddress
     * @param _driverAddress - Pass the driver address
     */
    function allNftIdsPerAddress(address _driverAddress) external view returns(uint[] memory allNFTIds){
        return allNFTIdToUser[_driverAddress];
    }
