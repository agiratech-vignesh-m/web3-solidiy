...
contract DreamLighter {
    ...
    function editBroker(uint256 id, string memory firstName, string memory lastName, address _address) public onlyAdmin{
        require(brokerStruct[id].id == id, "Broker does not exist");
        // if (linkBrokersAddress[_address]){
        //     revert AddressIsAlreadyExist();
        // }
        
        bytes32 encodeOldFirstName = keccak256(abi.encodePacked(brokerStruct[id].firstName));
        bytes32 encodeNewFirstName = keccak256(abi.encodePacked(firstName));
        bytes32 encodeOldLastName = keccak256(abi.encodePacked(brokerStruct[id].lastName));
        bytes32 encodeNewLastName = keccak256(abi.encodePacked(lastName));
        

        if(encodeOldFirstName != encodeNewFirstName && encodeOldLastName != encodeNewLastName && brokerStruct[id]._address != _address){
            brokerStruct[id].firstName = firstName;
            brokerStruct[id].lastName = lastName;
            brokerStruct[id]._address = _address;
            // linkBrokersAddress[_address] = true;
        }
        if (encodeOldFirstName != encodeNewFirstName && encodeOldLastName != encodeNewLastName){
            brokerStruct[id].firstName = firstName;
            brokerStruct[id].lastName = lastName;
        }
        if (encodeOldFirstName != encodeNewFirstName && brokerStruct[id]._address != _address) {
            brokerStruct[id].firstName = firstName;
            brokerStruct[id]._address = _address;
            // linkBrokersAddress[_address] = true;
        }
        if (encodeOldLastName != encodeNewLastName && brokerStruct[id]._address != _address) {
            brokerStruct[id].lastName = lastName;
            brokerStruct[id]._address = _address;
            // linkBrokersAddress[_address] = true;
        }
    ...
    {   
        ...
        return(
            broker.id, broker.firstName, broker.lastName, broker._address
        );
}

     function allBrokerDetails()
        public
        view
        returns (Broker[] memory _details)
    {
